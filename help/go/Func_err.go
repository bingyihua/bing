...
func cdr_check_data (str string) (errdata string){
...
func F071(str string,IMSI,UNICOM,PREFIX,AREA_CD map[string]string) (result string){
	...
	myres:=check_mobile(mobile_num[0:7],call_start_tm, IMSI,UNICOM,PREFIX,AREA_CD)
...
func save_errcd(file_list [] string,IMSI,UNICOM,PREFIX,AREA_CD,swich,sp map[string]string){
	rownum:=0
	/*rm file
	_, error := os.Stat(err)
	if error == nil {
...
func main() {
	...
	strlist:=readline_list("201908_100_err.csv")
	save_errcd(strlist,IMSI,UNICOM,PREFIX,AREA_CD,swich,sp)
